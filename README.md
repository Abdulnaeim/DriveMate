...
A Chrome extension that enables you to securely download shared videos from Google Drive by detecting and extracting their stream URLs.
...
DriveMate Downloader hooks into Chrome’s debugging protocol to monitor network requests made by Google Drive’s video player. When you navigate to a Drive video (even if it’s shared privately), the extension captures the media streaming data, extracts the highest‑quality progressive download URL, and presents it in a simple popup for one‑click downloading.
...
- **Reload Button**: Quickly reload the current browser tab if streams aren’t detected initially.

### Installation

1. Clone or download this [repository](https://github.com/TariqKingCoder/DriveMate/archive/refs/heads/main.zip).
...
5. The “DriveMate Downloader” icon will appear in your toolbar.
